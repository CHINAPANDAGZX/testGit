...
<%-- <script type="text/javascript" charset="UTF-8" src=<%=basepath+"/js/moment-with-locales.js" %>></script> --%>
<%-- <script type="text/javascript" charset="UTF-8" src=<%=basepath+"/js/bootstrap-datetimepicker.min.js" %>></script> --%>
<%-- <script type="text/javascript" charset="UTF-8" src=<%=basepath+"/js/bootstrap-datetimepicker.zh-CN.js" %>></script> --%>
<!-- laydate控件方式,layDate 采用原生 JavaScript 编写，不依赖任何第三方库，兼容所有浏览器（IE6/7除外） -->
<script src=<%=basepath+"/js/laydate/laydate.js" %>></script> <!-- 改成你的路径 -->

<script type="text/javascript" src="<%=basepath%>/js/my/my_doc_search.js" ></script>  
...
<!-- 	查询条件框部分开始 -->
	<div >
		<div>查询条件：</div>
		<div>
			<input type="hidden" id="uesr_id" value="${sessionScope.user.user_id}" />
			<label>文档名称：</label>
			...
			<!-- 按钮触发模态框 -->
			<button class="btn btn-primary" data-toggle="modal" data-target="#myModal1">上传文档</button>	
		</div>
		<div>
			<label>上传日期：</label>
...
<!-- 	查询结果表格部分开始 -->
	<div>
		<table class="table table-striped" id="myShowtab" >
			<tr>
				<td>序号</td><td>文档名称</td><td>文件类型</td><td>上传者</td><td>状态</td><td>操作</td>
			</tr>
		</table>
	</div>
<!-- 	查询结果表格部分结束 -->
