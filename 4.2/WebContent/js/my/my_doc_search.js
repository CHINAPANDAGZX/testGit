...
//	alert('点击函数触发');
	//页面查询部分参数
	var uesr_id = document.getElementById("uesr_id").value;
...
//					}
					if(j==0){
						...
						tdNode.innerHTML = cardList[i].d_name;	//文档名
					}else if(j==2){
						tdNode.innerHTML = cardList[i].d_type;		//文件类型
					}else if(j==3){
						tdNode.innerHTML = cardList[i].d_up_userid;		//上传员工ID
					}else if(j==4){
						tdNode.innerHTML = "暂无";
					}else if(j==5){
...
//							myDelete(this);
						...
						var nodea = document.createTextNode("下载连接");
						abtn.appendChild(nodea);
						abtn.setAttribute("href","/upload/qq.zip");
...
//	alert("当后changeEmp_Enable："+changeEmp_Enable);
	...
	$.ajax({
		type:"POST",
//		url:"",
		url:"../Emp_Servlet?action=changeEmpEnableApply",
		data:"changeEmp_ID=" + changeEmp_ID + 
